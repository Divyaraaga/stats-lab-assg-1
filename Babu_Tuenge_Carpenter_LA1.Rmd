...
```{r load}
cdebt_original = read.csv("CandidateDebt.csv")
...
Jason to Divya: I think these merit more analysis before including above, i.e., we need to demonstrate what we state.  
    * reportnumber: Sequentially-assigned unique value for each row presumably does not correlate with debt. We verified in Excel that the minimum difference between ordered values is one, so all values are unique. 
    * firstname, middleinitial, lastname: filerid and/or filername can presumably be used in lieu of these 3 variables 
...
``` 
```{r reportnumber}
# Jason to Divya: I can't get this to work... I'm trying to show there are 1043 levels (i.e., all unique)
#summary(as.factor(cdebt_all$reportnumber))
summary(as(cdebt_all$reportnumber, "factor")) 
``` 
...
# Jason to Divya: I went ahead and set some breakpoints. 
