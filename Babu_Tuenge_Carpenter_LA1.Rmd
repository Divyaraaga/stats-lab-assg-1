...
1. We were given a CSV which had a column and data mismatch at column L , 
   We created a new variable called "office2" next to office and moved the columns from legislativedistrict to left
...
3. At the first glance , The dataset contains few variables which does not contribute much to the analysis 
    1. reportnumber : Unique value does not conribute for analysis
    2. origin  : The column value is same for all the rows
```{r origin}
summary(cdebt_all$origin)
``` 
    3. filertype : The column value is same for all the rows 
```{r filertype}
summary(cdebt_all$filertype)
``` 
    4. filername, firstname, middleinitial, lastname:
       filerid can act on behalf of all these 4 variables
    5. vendoraddress , vendorcity ,vendorstate , vendorzip: 
       vendorname can act on behalf of all these variables
    6. electionyear: The column value is either 2012 or NA
```{r electionyear}
summary(cdebt_all$electionyear)
``` 
    7. recordtype :  The column value is either DEBT or NA
```{r recordtype}
...
Since we are trying to understand how campaign characteristics are related to the candidates debt, we look into the last row and last column where amount represents the x-axis and the other int or num variables represent the y-axis.

We noticed the following relationship between medv and the other variables:
...
#### Examine target variable : amount
```{r amount}
...
```{r}
#histinfo<-hist(cdebt_all$amount)
#histinfo
...
Most of the debt is around 0 to 6000 $ 

#### Examine target variable : filerid 
...
```{r filername}
summary(cdebt_all$filerid)
```

```{r}
filerid <- table(cdebt_all$filerid)

#Sets plot margins : b, l, t, r 
par(las=1, mar=c(2.5,8,0.5, 0.3))

barplot(filerid[order(filerid)], horiz = TRUE , las = 1, col = "#00A5DF", cex.names=0.4, main ="Debts wrt to Filers" , xlab="Number of debts", space = 0.05)
...
Debt wrt to filers is  and amount is  distributed. So the relationship between 
...
#Sets plot margins : b, l, t, r 
par(las=1, mar=c(2.5,7,2, 0.3))

barplot(party[order(party)], horiz = TRUE , las = 1, main ="Debts wrt to Filer's Party" , xlab="Number of debts" , cex.names= 0.9 , col = rainbow(16) ) 

#party.levels()
#length(party =="INDEPENDENT")

```

##### Univariate Analysis

We notice that maximum number of debt files are from Democrats and republicans

#### Examine the relationships between party and Debt Amount

```{r partyvspart}

#Sets outside margins : b, l, t, r 
par(las=1, mar=c(5,4,2,0.3))

boxplot(cdebt_all$amount ~ party, data = cdebt_all, cex.axis = .6, log='y',
main = " Debt distirbution wrt to party" , las=2 , col = "#BA554C")
