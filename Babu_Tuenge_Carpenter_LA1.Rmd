...
* There are 7 more levels for filerid than for filername. In some cases (but probably not all) this could be due to different candidates having the same name -- including middle initial.
```{r}
length(cdebt_original$reportnumber) - length(unique(cdebt_original$reportnumber))
length(unique(cdebt_original$filername)) - length(sort(unique(paste(cdebt_original$lastname, cdebt_original$firstname, cdebt_original$middleinitial)))) #shared lastname: FARRELL, JOHNSON, LADENBURG, MORRIS, THOMAS
...
Jason to others: Still working here...
```{r origin}
#summary(cdebt_revised)
c("legislativedistrict ", 1 - length(na.omit(cdebt_revised$legislativedistrict)) / length(cdebt_revised$legislativedistrict))
...
newFromdate = as.Date(cdebt_revised$fromdate, "%m/%d/%Y") + 365.25 * 2000 - 15
newThrudate = as.Date(cdebt_revised$thrudate, "%m/%d/%Y") + 365.25 * 2000 - 15
newDebtdate = as.Date(cdebt_revised$debtdate, "%m/%d/%Y") + 365.25 * 2000 - 15
summary(newThrudate)
``` 

Following are some additional observations made while repairing the CSV file in Excel:
* A given filername is often associated with a variety of parties and offices. In some cases (but probably not all) this could be due to different candidates having the same name; this might for instance apply to filernames with more than one filerid. 
...
Divya to Jason : Sure , will see if i can plot ggraphs
...
# Divya to Jason : As date variables have new names, just modified the names for the plots 
# Divya to Jason : This is what u r trying to say i believe . Increase in the no of debts as we progress thru the years. Any of the below 2 graphs should help you. 

plot(cdebt_revised$reportnumber, newFromdate)
plot(cdebt_revised$reportnumber, newDebtdate)

#Divya to Jason : You can ignore below 3 plots i guess

#plot(newFromdate, amount)
#plot(newThrudate, amount)
#plot(newDebtdate, amount)
...
main = " Debt distirbution by party", col = colorBAPalette)
...
### Examine the relationships between amount and office and party
